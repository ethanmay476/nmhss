...
# Assuming p_values_df is already created and contains p-values for various tests

# Function to apply LaTeX coloring based on p-value significance
apply_coloring <- function(p_value) {
  if (!is.na(p_value) && p_value > 0.05) {
    # Applying green color for significant p-values
    return(sprintf("\\textbf{\\textit{%.3f}}", p_value))
  } else {
    # Formatting non-significant p-values without coloring
    return(sprintf("%.3f", p_value))
  }
}
# "\\textbf{\\textit{%.3f}}"
#"\\cellcolor{green}{%.3f}"
# Apply this function to each p-value in your dataframe
for (col in names(p_values_df)[-1]) { # Assuming the first column is not p-values but variable names or similar
  p_values_df[col] <- sapply(p_values_df[[col]], function(x) ifelse(x == "-", x, apply_coloring(as.numeric(x))))
}
...
# when using green get rid of striped command
final_summary_table <- kable(p_values_df, format = "latex", longtable = TRUE, booktabs = TRUE, caption = "P-values by SRVC62 and Year", escape = FALSE) %>%
